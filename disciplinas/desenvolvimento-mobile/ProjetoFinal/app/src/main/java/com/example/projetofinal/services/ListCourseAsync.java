...
public class ListCourseAsync extends AsyncTask<String,String,String> {

    private String metodo;
    ProgressDialog progressDialog;
    Context context;

    public ListCourseAsync(String metodo, Context context){
        this.metodo = metodo;
    ...
    protected void onPostExecute(String s) {
        ...
        if(metodo.equals("GET")){
            ((MainActivity)context).setListaCourse(Course.parseArrayList(s));
            ((MainActivity)context).setupRecyclerCourses();
            progressDialog.dismiss();
        } else if(s.equals("OK")){
            Toast.makeText(context,"Operação realizada com sucesso",Toast.LENGTH_SHORT)
                    .show();
            progressDialog.dismiss();
            ((MainActivity)context).buscarCourses();
        }
